//
// Copyright (c) 2023 Ruben Perez Hidalgo (rubenperez038 at gmail dot com)
//
// Distributed under the Boost Software License, Version 1.0. (See accompanying
// file LICENSE_1_0.txt or copy at http://www.boost.org/LICENSE_1_0.txt)
//

#include "redis_serialization.hpp"

#include <boost/redis/resp3/node.hpp>
#include <boost/redis/resp3/type.hpp>
#include <boost/redis/response.hpp>
#include <boost/system/error_code.hpp>
#include <boost/test/unit_test.hpp>

#include <chrono>

#include "business.hpp"

namespace resp3 = boost::redis::resp3;
using namespace chat;

BOOST_AUTO_TEST_SUITE(redis_serialization)

static resp3::node array_node(std::size_t size, std::size_t depth)
{
    return {resp3::type::array, size, depth, ""};
}

static resp3::node string_node(std::size_t depth, std::string content)
{
    return {
        resp3::type::blob_string,
        0,
        depth,
        std::move(content),
    };
}

BOOST_AUTO_TEST_CASE(parse_room_history_success)
{
    std::vector<resp3::node> nodes{
        // top-level node
        array_node(2, 0),
        // first message
        array_node(2, 1),
        string_node(2, "100-1"),
        array_node(2, 2),
        string_node(3, "payload"),
        string_node(
            3,
            R"%({"user":{"id":"user1","username":"username1"},"content":"Test message 2","timestamp":1691666793896})%"
        ),
        // second message
        array_node(2, 1),
        string_node(2, "90-1"),
        array_node(2, 2),
        string_node(3, "payload"),
        string_node(
            3,
            R"%({"user":{"id":"user2","username":"username2"},"content":"Test message 3","timestamp":1691666793897})%"
        ),
    };

    // Call the function
    auto res = parse_room_history(nodes);
    const auto& val = res.value();

    // Validate
    BOOST_TEST(val.size() == 2u);

    BOOST_TEST(val[0].id == "100-1");
    BOOST_TEST(val[0].usr.id == "user1");
    BOOST_TEST(val[0].usr.username == "username1");
    BOOST_TEST(serialize_timestamp(val[0].timestamp) == 1691666793896);
    BOOST_TEST(val[0].content == "Test message 2");

    BOOST_TEST(val[1].id == "90-1");
    BOOST_TEST(val[1].usr.id == "user2");
    BOOST_TEST(val[1].usr.username == "username2");
    BOOST_TEST(serialize_timestamp(val[1].timestamp) == 1691666793897);
    BOOST_TEST(val[1].content == "Test message 3");
}

BOOST_AUTO_TEST_CASE(parse_room_history_empty)
{
    std::vector<resp3::node> nodes{
        // top-level node
        array_node(0, 0),
    };

    // Call the function
    auto res = parse_room_history(nodes);
    const auto& val = res.value();

    // Validate
    BOOST_TEST(val.size() == 0u);
}

BOOST_AUTO_TEST_CASE(parse_room_history_error)
{
    std::vector<resp3::node> nodes{
        // top-level node
        array_node(1, 0),
        // first message
        array_node(2, 1),
        string_node(2, "100-1"),
        array_node(2, 2),
        string_node(3, "invalid"),
        string_node(
            3,
            R"%({"user":{"id":"user1","username":"username1"},"content":"Test message 2","timestamp":1691666793896})%"
        ),
    };

    // Call the function
    auto res = parse_room_history(nodes);
    BOOST_TEST(res.error() == error_code(errc::redis_parse_error));
}

BOOST_AUTO_TEST_CASE(parse_room_history_batch_success)
{
    std::vector<resp3::node> nodes{
        // 1st room top-level node
        array_node(2, 0),
        // first message
        array_node(2, 1),
        string_node(2, "100-1"),
        array_node(2, 2),
        string_node(3, "payload"),
        string_node(
            3,
            R"%({"user":{"id":"user1","username":"username1"},"content":"Test message 2","timestamp":1691666793896})%"
        ),
        // second message
        array_node(2, 1),
        string_node(2, "90-1"),
        array_node(2, 2),
        string_node(3, "payload"),
        string_node(
            3,
            R"%({"user":{"id":"user2","username":"username2"},"content":"Test message 3","timestamp":1691666793897})%"
        ),

        // 2nd room top-level node; room has no messages
        array_node(0, 0),

        // 3rd room top-level node
        array_node(1, 0),
        // first message
        array_node(2, 1),
        string_node(2, "150-1"),
        array_node(2, 2),
        string_node(3, "payload"),
        string_node(
            3,
            R"%({"user":{"id":"user1","username":"username1"},"content":"msg7","timestamp":1691666793898})%"
        ),
    };

    // Call the function
    auto res = parse_room_history_batch(nodes);
    const auto& val = res.value();

    // Validate
    BOOST_TEST(val.size() == 3u);
    BOOST_TEST(val[0].size() == 2u);
    BOOST_TEST(val[1].size() == 0u);
    BOOST_TEST(val[2].size() == 1u);

    BOOST_TEST(val[0][0].id == "100-1");
    BOOST_TEST(val[0][0].usr.id == "user1");
    BOOST_TEST(val[0][0].usr.username == "username1");
    BOOST_TEST(serialize_timestamp(val[0][0].timestamp) == 1691666793896);
    BOOST_TEST(val[0][0].content == "Test message 2");

    BOOST_TEST(val[0][1].id == "90-1");
    BOOST_TEST(val[0][1].usr.id == "user2");
    BOOST_TEST(val[0][1].usr.username == "username2");
    BOOST_TEST(serialize_timestamp(val[0][1].timestamp) == 1691666793897);
    BOOST_TEST(val[0][1].content == "Test message 3");

    BOOST_TEST(val[2][0].id == "150-1");
    BOOST_TEST(val[2][0].usr.id == "user1");
    BOOST_TEST(val[2][0].usr.username == "username1");
    BOOST_TEST(serialize_timestamp(val[2][0].timestamp) == 1691666793898);
    BOOST_TEST(val[2][0].content == "msg7");
}

BOOST_AUTO_TEST_CASE(parse_room_history_batch_empty)
{
    std::vector<resp3::node> nodes{};

    // Call the function
    auto res = parse_room_history_batch(nodes);
    const auto& val = res.value();

    // Validate
    BOOST_TEST(val.size() == 0u);
}

BOOST_AUTO_TEST_CASE(parse_room_history_batch_error)
{
    std::vector<resp3::node> nodes{
        // top-level node
        array_node(1, 0),
        // first message
        array_node(2, 1),
        string_node(2, "100-1"),
        array_node(2, 2),
        array_node(0, 0),  // this top-level node shouldn't be here
        string_node(3, "payload"),
        string_node(
            3,
            R"%({"user":{"id":"user1","username":"username1"},"content":"Test message 2","timestamp":1691666793896})%"
        ),
    };

    // Call the function
    auto res = parse_room_history_batch(nodes);
    BOOST_TEST(res.error() == error_code(errc::redis_parse_error));
}

BOOST_AUTO_TEST_CASE(parse_string_list_success)
{
    std::vector<resp3::node> nodes{string_node(0, "s1"), string_node(0, "s2"), string_node(0, "mykey")};

    // Call the function
    auto res = parse_string_list(nodes);
    auto& val = res.value();

    // Validate
    BOOST_TEST(val == std::vector<std::string>({"s1", "s2", "mykey"}));
}

BOOST_AUTO_TEST_CASE(parse_string_list_empty)
{
    std::vector<resp3::node> nodes{};

    // Call the function
    auto res = parse_string_list(nodes);
    auto& val = res.value();

    // Validate
    BOOST_TEST(val.size() == 0u);
}

BOOST_AUTO_TEST_CASE(parse_string_list_error)
{
    std::vector<resp3::node> nodes{
        string_node(0, "s1"),
        array_node(1, 0),  // This shouldn't be here
        string_node(0, "s1"),
    };

    // Call the function
    auto res = parse_string_list(nodes);
    BOOST_TEST(res.error() == error_code(errc::redis_parse_error));
}

BOOST_AUTO_TEST_CASE(serialize_redis_message_success)
{
    message input{
        "100-10",
        "hello world!",
        user{"user1", "name1"},
        timestamp_t{std::chrono::milliseconds(123)},
    };

    auto output = serialize_redis_message(input);

    const char*
        expected = R"%({"user":{"id":"user1","username":"name1"},"content":"hello world!","timestamp":123})%";
    BOOST_TEST(output == expected);
}

BOOST_AUTO_TEST_SUITE_END()