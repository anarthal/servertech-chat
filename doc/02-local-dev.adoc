...
enabled (if you change something in the client files, it will hot-reload your
code). By default, the client attempts to connect to your pass:[C++] server
on port 8080. You can change this setting by editing the `client/.env.local`
file.
...
as expected by exercising the websocket API directly. You need the
pass:[C++] server running on port 8080 to run them - the client is not required.
